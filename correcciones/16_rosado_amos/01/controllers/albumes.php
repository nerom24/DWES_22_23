        ...
        function borrar_directorio($dirname){
            ...
            while ($file = readdir($dir_handle)) {
                if ($file != "." && $file != "..") {
                    ...
                    if (!is_dir($dirname . "/" . $file)) //Aqui teniais imagenes 
                        unlink($dirname . "/" . $file);
            ...
            if (!isset($_SESSION['id'])) {
                ...
                foreach ($ficheros['name'] as $key => $fichero) {
                    if ($ficheros['error'][$key] === UPLOAD_ERR_OK) {
                        if (!is_uploaded_file($ficheros['tmp_name'][$key])) {
                            $errores['archivo'] = 'Error en la subida del archivo';
                        } else {
                            $max_size = 2 * 1024 * 1024;
                            if ($ficheros['size'][$key] > $max_size) {
                ...
                if (!empty($errores)) {
                    ...
                    move_uploaded_file($ficheros['tmp_name'], 'imagenes2/' . $this->view->album->carpeta . "/" . $ficheros['name']);
